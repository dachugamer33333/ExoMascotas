...
export default function HotelCard({ hotel }: HotelCardProps) {
  return (
    <div className="group relative h-full">
      ...
      <div className="relative bg-white/90 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 border border-white/20 h-full flex flex-col">
        {/* Hotel Image */}
        ...
        <div className="p-8 flex flex-col flex-1">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-emerald-50 p-4 rounded-2xl mb-6 border border-blue-100/50">
            <div className="flex items-start space-x-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-blue-700 text-sm leading-relaxed line-clamp-2">
                  {hotel.politica}
                </p>
              </div>
            </div>
          ...
          <div className="mb-6 flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              {hotel.servicios.slice(0, 6).map((servicio, index) => (
                <span 
                  key={index}
                  className="bg-gray-100 hover:bg-emerald-100 text-gray-800 hover:text-emerald-700 text-xs px-3 py-2 rounded-xl transition-colors duration-300 border border-gray-200 hover:border-emerald-200"
                >
                  {servicio}
                </span>
              ))}
              {hotel.servicios.length > 6 && (
                <span className="bg-emerald-100 text-emerald-700 text-xs px-3 py-2 rounded-xl border border-emerald-200">
